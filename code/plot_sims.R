...
factor_order_geno <- str_c("Genotype Distribution\n", factor_order_vec)
...
sims %>%
  ...
  mutate(issame = geno_dist == Prior,
         geno_dist = recode(geno_dist,
                            ...
                            "flex"    = "General",
                            "uniform" = "Uniform"),
         geno_dist = str_c("Genotype Distribution\n", geno_dist),
         Prior = recode(Prior,
                        "hw"      = "Hardy-Weinberg",
                        "bb"      = "Beta-binomial",
                        "norm"    = "Normal",
                        ...
                        "flex"    = "General",
                        "uniform" = "Uniform"),
         geno_dist = parse_factor(geno_dist, levels = factor_order_geno),
         Prior = parse_factor(Prior, levels = factor_order_vec)) ->
  simslong

## Plot Proportion correct ----------------------------------------------------
for (index in seq_along(factor_order_vec)) {
  current_geno_dist <- factor_order_geno[index]
  current_geno_val <- factor_order_vec[index]
  simslong %>%
    ...
    xlab("Assumed Prior During Fit") +
    ylab("Proportion Correct") ->
    pl
  ggsave(filename = paste0("./output/figures/pc_plots/pc_", current_geno_val, ".pdf"),
         plot = pl,
         height = 7,
         width = 6,
         family = "Times")
...
for (index in seq_along(factor_order_vec)) {
  ...
  simslong %>%
    ...
    scale_y_log10() ->
    pl
  ggsave(filename = paste0("./output/figures/bias_plots/bias_", current_geno_val, ".pdf"),
         plot = pl,
         height = 7,
         width = 6,
         family = "Times")
...
for (index in seq_along(factor_order_vec)) {
  ...
  simslong %>%
    ...
    pl
  ggsave(filename = paste0("./output/figures/od_plots/od_", current_geno_val, ".pdf"),
         plot = pl,
         height = 7,
         width = 6,
         family = "Times")
...
for (index in seq_along(factor_order_vec)) {
  ...
  simslong %>%
    ...
    pl
  ggsave(filename = paste0("./output/figures/epm_plots/epm_", current_geno_val, ".pdf"),
         plot = pl,
         height = 7,
         width = 6,
         family = "Times")
